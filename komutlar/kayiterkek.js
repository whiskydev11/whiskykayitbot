...
exports.run = async(client, message, args) => {
  ...
  let erkekrol = db.get(`erkekrol_${sunucu.id}`)
  let isim = args[1]
  ...
  if (erkekrol === null) return message.channel.send("Erkek Rolü Ayarlanmamış")
  
  if (kayıtsızrol === null) return message.channel.send("Kayıtsız Rolü Ayarlanmamış")
  
  if (sohbetkanal === null) return message.channel.send("Kayıt Bitiş Kanalı Ayarlanmamış")
